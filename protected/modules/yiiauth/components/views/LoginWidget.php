	...
	foreach ($providers as $provider){
		echo CHtml::link('<img src="'.Yii::app()->request->baseUrl.$path.$provider.'.png" alt="'.$provider.'" title="'.$provider.'" />',
	...
	foreach($openid_servers as $image=>$server){
		echo CHtml::link('<img src="'.Yii::app()->request->baseUrl.$path.$image.'.png" alt="'.$server.'" title="'.$server.'"/>',
