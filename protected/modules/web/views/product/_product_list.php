...
foreach ($products as $product) {
    ...
    <div class="condition">
        <?php
        $image = "noimages.jpeg";
        if (isset($product['image'][0]['image_large'])) {
            $image = $product['image'][0]['image_large'];
        }
        echo CHtml::link(CHtml::image(Yii::app()->baseUrl . '/images/product_images/' . $image, 'condition'), $this->createUrl('/web/product/productDetail', array('country' => Yii::app()->session['country_short_name'], 'city' => Yii::app()->session['city_short_name'], 'city_id' => Yii::app()->session['city_id'], 'product_id' => $product['product_id'])));
        ?>
        <h3>
            <?php
