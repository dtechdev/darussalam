    ...
    <div class="condition" style="text-align: center">
        ...
        <h3>
            ...
            if (Yii::app()->controller->action->id == "getSearch") {
                ...
                echo CHtml::link(implode(' ', array_slice(explode(' ', $product['product_name']), 0, 4)), $this->createUrl('/web/educationToys/productDetail', array('country' => Yii::app()->session['country_short_name'], 'city' => Yii::app()->session['city_short_name'], 'city_id' => Yii::app()->session['city_id'], 'product_id' => $product['product_id'])), array('title' => $product['product_name']));
            }
            ?>
        </h3>
        <p>
